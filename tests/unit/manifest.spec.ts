...
describe("manifest helpers", () => {
  it("writes and reads a manifest from the project-local .product-kit directory", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "product-kit-manifest-"));
    ...
    expect(loaded?.productKitVersion).toBe("0.1.0");
